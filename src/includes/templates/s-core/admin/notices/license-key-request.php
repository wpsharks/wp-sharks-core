...
?>
<?php if (($trial_days_remaining = $this->s::trialDaysRemaining()) >= 0) : ?>
    ...
    <?php if ($this->s::isTrialExpired()) : // Trial applicable.?>
        ...
        <form method="post" action="<?= esc_url($this->App->s::brandUrl()); ?>" target="_blank" style="margin:0;">
            <p>
                ...
                <?= $this->s::menuPageTip(sprintf(__('Click the \'Buy Now\' button to purchase a license key.<hr />At %1$s™, navigate to your \'My Downloads\' page to get your license key.', 'wp-sharks-core'), esc_html($this->App::CORE_CONTAINER_NAME))); ?>
            </p>
        </form>
        ...
        <form method="post" action="<?= esc_url($this->s::dismissNoticeUrl('§license-key-request')); ?>" style="margin:.5em 0 1em 0;">
            ...
            <?php if ($trial_days_remaining === $this->s::trialDays()) : ?>
                <button type="submit" class="button button-small" style="vertical-align:middle;">
                    <?= __('Dismiss &amp; Begin Free Trial', 'wp-sharks-core'); ?>
                </button>
